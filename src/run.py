...
**何を落とすかはここに書かない。** src 側の管理表（レポート管理表.xlsx）で
...
**1 日に何度も欲しいレポートは、「スケジュール」シートにスケジュールキーを
分けて複数行登録すれば、このバッチの中で完結する。**

このバッチとは別に、今すぐの最新値（定期取得済みキャッシュ）だけを読みたい
下流プログラムは、このバッチを増やすのではなく `src.paths.output_path()`
が組み立てたパスを直接 `pathlib.Path.glob()` などで探せばよい。
